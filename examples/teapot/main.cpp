    ...
    const std::shared_ptr<simplex::core::graphics::RendererBase>& renderer)
{
    ...
    cameraNode->setTransform(
        simplex::utils::Transform::makeLookAt(glm::vec3(0.f, 0.f, 2.f), glm::vec3(0.f), glm::vec3(0.f, 1.f, 0.f)).inverted());
...
int main(int argc, char* argv[])
{
    ...
    window->setUpdateCallback(
        [](uint64_t time, uint32_t)
        {
            ...
            if (auto it = std::find_if(
                    ...
                    [](const std::shared_ptr<simplex::core::Node>& node) { return node->name() == "TeapotNode"; });
                it != nodes.end())
            {
                (*it)->setTransform(
    ...
    app.setScene(createScene("Teapot", window->graphicsEngine()->graphicsRenderer()));
