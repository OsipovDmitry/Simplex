...
static int MouseStandardCursorToGLFWMouseStandardCursor(core::graphics::MouseStandardCursor value)
{
    static std::array<int, core::graphics::numElementsMouseStandardCursor()> s_table{
        GLFW_ARROW_CURSOR,     GLFW_IBEAM_CURSOR,       GLFW_CROSSHAIR_CURSOR,   GLFW_HAND_CURSOR,       GLFW_RESIZE_EW_CURSOR,
        GLFW_RESIZE_NS_CURSOR, GLFW_RESIZE_NWSE_CURSOR, GLFW_RESIZE_NESW_CURSOR, GLFW_RESIZE_ALL_CURSOR, GLFW_NOT_ALLOWED_CURSOR};
...
static void closeWidget(const GLFWWidget& widget)
{
    ...
    if (auto callback = widget.closeCallback()) callback();
...
static std::shared_ptr<GLFWWidget> findGLFWWidget(GLFWwindow* window)
{
    ...
    {
        if (widgetWeak.expired()) continue;
...
static void GLFWResizeCallback(GLFWwindow* window, int width, int height)
{
    ...
    if (auto callback = widget->resizeCallback()) callback(size);
...
static void GLFWKeyCallback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
    ...
    if (auto callback = widget->keyCallback()) callback(keyState, keyCode, keyModifiers);
}
...
static void GLFWMouseCursorMoveCallback(GLFWwindow* window, double x, double y)
{
    ...
    if (auto callback = widget->mouseCursorMoveCallback()) callback(glm::ivec2(x, y));
}
...
static void GLFWMouseCursorEnterCallback(GLFWwindow* window, int isEntered)
{
    ...
    if (auto callback = widget->mouseCursorEnterCallback()) callback(isEntered);
}
...
static void GLFWMouseButtonCallback(GLFWwindow* window, int button, int action, int mods)
{
    ...
    if (auto callback = widget->mouseButtonCallback()) callback(mouseButtonState, mouseButton, keyModifiers);
}
...
static void GLFWMouseScrollCallback(GLFWwindow* window, double x, double y)
{
    ...
    if (auto callback = widget->mouseScrollCallback()) callback(glm::ivec2(x, y));
}
...
{
    if (auto& callback = m_->updateCallback()) callback(time, dt);
...
void GLFWWidget::setIcon(const std::shared_ptr<utils::Image>& image)
{
    ...
    if (image)
    {
        imageRGBA8 = image->converted(image->width(), image->height(), 4u, utils::PixelComponentType::Uint8);
...
void GLFWWidget::setMouseCursor(const std::shared_ptr<utils::Image>& image, const glm::uvec2& hotPoint)
{
    ...
    auto imageRGBA8 = image->converted(image->width(), image->height(), 4u, utils::PixelComponentType::Uint8);
...
{
    return glfwGetKey(m_->window(), KeyCodeToGLFWKeyCode(value)) == GLFW_PRESS ? core::graphics::KeyState::Pressed
                                                                               : core::graphics::KeyState::Released;
}
...
core::graphics::MouseButtonState GLFWWidget::mouseButtonState(core::graphics::MouseButton value) const
{
    return glfwGetMouseButton(m_->window(), MouseButtonToGLFWMouseButton(value)) == GLFW_PRESS
               ? core::graphics::MouseButtonState::Pressed
               : core::graphics::MouseButtonState::Released;
}
...
std::shared_ptr<GLFWWidget> GLFWWidget::getOrCreate(const std::string& name, const std::shared_ptr<GLFWWidget>& sharedWidget)
{
    ...
    if (!result)
    {
        ...
        if (sharedWidget) shareGroup = sharedWidget->shareGroup();

        if (!shareGroup) shareGroup = std::make_shared<core::graphics::ShareGroup>();
        ...
        LOG_INFO << "GLFWWidget has been created (Name: \"" << name
                 << "\", OpenGL context ver:" << GLFWWidgetPrivate::majorVersion() << "." << GLFWWidgetPrivate::minorVersion()
                 << ")";
    }
    ...
    : m_(std::make_unique<GLFWWidgetPrivate>(name, shareGroup))
{
    if (!GLFWWidgetPrivate::isGLFWInitialized())
    {
        ...
        if (!tempWindow) LOG_CRITICAL << "Failed to create GLFW window " << name;
    ...
    if (!shareGroup) LOG_CRITICAL << "Share group can't be nullptr";
    ...
    if (auto it = shareGroup->find_any(); it != shareGroup->end())
    {
        ...
        if (!widget) LOG_CRITICAL << "Device \"" << it->lock()->name() << "\" can't be casted to GLFWWidget";
