...
public:
    ...
    static ClipSpaceT<T> makePerspective(
        T left,
        T right,
        T bottom,
...
inline typename ClipSpaceT<T>::MatrixType ClipSpaceT<T>::projectionMatrix(const utils::RangeT<T>& zRange) const
{
    ...
    if (zRange.isEmpty()) return result;

    switch (m_type)
    {
        case ClipSpaceType::Ortho:
        {
            result = glm::ortho(m_params[0u], m_params[1u], m_params[2u], m_params[3u], zRange.nearValue(), zRange.farValue());
            break;
        }
        case ClipSpaceType::Perspective:
        {
            ...
            result = glm::frustum(
                m_params[0u] * zNear, m_params[1u] * zNear, m_params[2u] * zNear, m_params[3u] * zNear, zRange.nearValue(),
                zRange.farValue());
            break;
        }
...
template <typename T>
inline ClipSpaceT<T> ClipSpaceT<T>::makeOrtho(T aspectRatio, T height)
{
...
template <typename T>
inline ClipSpaceT<T> ClipSpaceT<T>::makePerspective(T aspectRatio, T fovY)
{
