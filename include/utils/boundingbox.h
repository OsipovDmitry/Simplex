...
public:
    ...
    BoundingBoxT()
        : BoundingBoxT<L, T>(empty())
    {
    }
    BoundingBoxT(const std::array<PointType, 2u>& pp)
        : m_points(pp)
    {
    }
    ...
    PointType halfSizes() const { return static_cast<T>(.5) * (m_points[1u] - m_points[0u]); }
    ...
    static BoundingBoxT<L, T> fromMinMax(const PointType& minP, const PointType& maxP)
    {
        return BoundingBoxT<L, T>({minP, maxP});
    }
    static BoundingBoxT<L, T> fromCenterHalfSize(const PointType& cP, const PointType& hs)
    {
        return BoundingBoxT<L, T>({cP - hs, cP + hs});
    }
    static BoundingBoxT<L, T> empty()
    {
        return BoundingBoxT<L, T>(
            {PointType(std::numeric_limits<value_type>::max()), PointType(std::numeric_limits<value_type>::lowest())});
    }
...
inline std::array<size_t, 2u> BoundingBoxT<L, T>::edgeIndices(size_t index)
{
    ...
    return {
        std::bitset<L>(NthNumberWithEvenBitsCount(index / L)).to_ullong(),
...
{
    const auto dists = pairDistancesToPlane(p);
    return (dists.nearValue() * dists.farValue() <= static_cast<T>(0)) ? 0 : ((dists.nearValue() > static_cast<T>(0)) ? +1 : -1);
}

template <glm::length_t L, typename T>
inline BoundingBoxT<L, T>& BoundingBoxT<L, T>::operator+=(const BoundingBoxT<L, T>& b)
{
    *this = *this + b;
    return *this;
}
...
template <glm::length_t L, typename T>
inline BoundingBoxT<L, T>& BoundingBoxT<L, T>::operator+=(const typename BoundingBoxT<L, T>::PointType& v)
{
    *this = *this + v;
    return *this;
}
