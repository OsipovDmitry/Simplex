...
core::audio::BufferFormat Conversions::BitsAndChannels2BufferFormat(uint16_t bits, uint16_t channels)
{
    ...
    switch (channels)
    {
        case 1u:
        {
            ...
            {
                case 8u:
                    result = core::audio::BufferFormat::Mono8;
                    break;
                case 16u:
                    result = core::audio::BufferFormat::Mono16;
                    break;
                default:
                    break;
            }
        ...
        case 2u:
        {
            ...
            {
                case 8u:
                    result = core::audio::BufferFormat::Stereo8;
                    break;
                case 16u:
                    result = core::audio::BufferFormat::Stereo16;
                    break;
                default:
                    break;
            }
...
{
    static std::array<ALenum, core::audio::numElementsSourceState()> s_table{AL_INITIAL, AL_PLAYING, AL_PAUSED, AL_STOPPED};
...
void Buffer_1_1::setData(core::audio::BufferFormat format, const void* data, size_t dataSize, uint32_t frequency)
{
    CHECK_CURRENT_CONTEXT
    alBufferData(
        m_id, Conversions::BufferFormat2AL(format), data, static_cast<ALsizei>(dataSize), static_cast<ALsizei>(frequency));
}
...
{
    CHECK_CURRENT_CONTEXT
    if (value <= 0.f) LOG_CRITICAL << "Pitch can't be less or equal than 0.0";
...
{
    CHECK_CURRENT_CONTEXT
    if (value < 0.f) LOG_CRITICAL << "Gain can't be less than 0.0";
...
{
    CHECK_CURRENT_CONTEXT
    if (value < 0.f || value > 1.f) LOG_CRITICAL << "Min gain must be in [0.0..1.0]";
...
{
    CHECK_CURRENT_CONTEXT
    if (value < 0.f || value > 1.f) LOG_CRITICAL << "Max gain must be in [0.0..1.0]";
...
{
    CHECK_CURRENT_CONTEXT
    if (value < 0.f) LOG_CRITICAL << "Reference Distance can't be less than 0.0";
...
{
    CHECK_CURRENT_CONTEXT
    if (value < 0.f) LOG_CRITICAL << "Max Distance can't be less than 0.0";
...
{
    CHECK_CURRENT_CONTEXT
    if (value < 0.f) LOG_CRITICAL << "Rolloff Factor can't be less than 0.0";
...
{
    CHECK_CURRENT_CONTEXT
    if (value < 0.f) LOG_CRITICAL << "Cone Outer Gain can't be less than 0.0";
...
Listener_1_1::Listener_1_1(){SAVE_CURRENT_CONTEXT}
...
OpenAL_1_1_Renderer::OpenAL_1_1_Renderer(const std::string& name, const std::weak_ptr<OpenALDevice>& device)
    ...
    , m_device(device)
{
    if (m_device.expired()) LOG_CRITICAL << "OpenALDevice can't be nullptr";

    auto openALDevice = m_device.lock();
    if (!openALDevice) LOG_CRITICAL << "OpenALDevice can't be nullptr";

    m_context = alcCreateContext(openALDevice->m().device(), nullptr);
    if (!m_context) LOG_CRITICAL << "Failed to create OpenAL context";
...
std::shared_ptr<core::audio::IBuffer> OpenAL_1_1_Renderer::createBuffer(const std::shared_ptr<utils::Sound>& sound) const
{
    ...
    {
        result->setData(
            Conversions::BitsAndChannels2BufferFormat(sound->bitsPerSample(), sound->numChannels()), sound->data(),
            sound->numFrames() * sound->numChannels() * (sound->bitsPerSample() / 8u), sound->sampleRate());
    }
...
core::audio::SourceState OpenAL_1_1_Renderer::sourceState(const std::shared_ptr<core::audio::ISource>& source) const
{
    ...
    if (!source_1_1) LOG_CRITICAL << "Source can't be nullptr";
...
{
    if (m_context == alcGetCurrentContext()) return true;

    bool result = alcMakeContextCurrent(m_context) != 0;
    if (!result) LOG_ERROR << "Failed to make OpenAL context current";
...
bool OpenAL_1_1_Renderer::doDoneCurrent()
{
    ...
    if (!result) LOG_ERROR << "Failed to done OpenAL context current";
