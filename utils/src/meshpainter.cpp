...
template <typename V>
inline void calculateNormalImpl(const std::shared_ptr<DrawArrays>& drawArrays, const VertexBuffer& vertices, VertexBuffer& normals)
{
    calculateNormal(
        drawArrays, vertices.numComponents(), reinterpret_cast<const V*>(vertices.data()), normals.numComponents(),
        reinterpret_cast<V*>(normals.data()));
}

template <typename V, typename I>
inline void calculateNormalImpl(
...
{
    calculateNormal(
        drawElements, vertices.numComponents(), reinterpret_cast<const V*>(vertices.data()),
        reinterpret_cast<const I*>(indices.data()), normals.numComponents(), reinterpret_cast<V*>(normals.data()));
}

template <typename V>
inline void calculateNormalImpl(
    const std::shared_ptr<DrawElementsBuffer>& drawElementsBuffer,
    const VertexBuffer& vertices,
    VertexBuffer& normals)
{
...
template <typename V>
inline void normalizeImpl(size_t numVertices, const VertexBuffer& normals, VertexBuffer& result)
{
    normalize(
        numVertices, normals.numComponents(), reinterpret_cast<const V*>(normals.data()), result.numComponents(),
        reinterpret_cast<V*>(result.data()));
}

template <typename V>
inline void calculateTangentSpaceImpl(
    ...
    VertexBuffer& binormals)
{
    calculateTangentSpace(
        drawArrays, vertices.numComponents(), reinterpret_cast<const V*>(vertices.data()), texCoords.numComponents(),
        reinterpret_cast<const V*>(texCoords.data()), tangents.numComponents(), reinterpret_cast<V*>(tangents.data()),
        binormals.numComponents(), reinterpret_cast<V*>(binormals.data()));
}
...
{
    calculateTangentSpace(
        drawElements, vertices.numComponents(), reinterpret_cast<const V*>(vertices.data()), texCoords.numComponents(),
        reinterpret_cast<const V*>(texCoords.data()), reinterpret_cast<const I*>(indices.data()), tangents.numComponents(),
        reinterpret_cast<V*>(tangents.data()), binormals.numComponents(), reinterpret_cast<V*>(binormals.data()));
}
    ...
    VertexBuffer& binormals)
{
    ...
    {
        case DrawElementsIndexType::Uint8:
            calculateTangentSpaceImpl<V, uint8_t>(
                drawElementsBuffer, vertices, texCoords, *drawElementsBuffer, tangents, binormals);
            break;
        case DrawElementsIndexType::Uint16:
            calculateTangentSpaceImpl<V, uint16_t>(
                drawElementsBuffer, vertices, texCoords, *drawElementsBuffer, tangents, binormals);
            break;
        case DrawElementsIndexType::Uint32:
            calculateTangentSpaceImpl<V, uint32_t>(
                drawElementsBuffer, vertices, texCoords, *drawElementsBuffer, tangents, binormals);
            break;
        default:
            LOG_ERROR << "Undefined draw elements buffer type";
...
template <typename V>
inline void orthogonalizeTangentSpaceImpl(
    size_t numVertices,
    ...
    VertexBuffer& result)
{
    orthogonalizeTangentSpace(
        numVertices, tangents.numComponents(), reinterpret_cast<const V*>(tangents.data()), binormals.numComponents(),
        reinterpret_cast<const V*>(binormals.data()), normals.numComponents(), reinterpret_cast<const V*>(normals.data()),
        result.numComponents(), reinterpret_cast<V*>(result.data()));
}

template <typename V>
inline BoundingBox calculateBoundingBoxImpl(const std::shared_ptr<DrawArrays>& drawArrays, const VertexBuffer& vertices)
{
    return calculateBoundingBox<V>(drawArrays, vertices.numComponents(), reinterpret_cast<const V*>(vertices.data()));
}

template <typename V, typename I>
inline BoundingBox calculateBoundingBoxImpl(
    const std::shared_ptr<DrawElements>& drawElements,
    const VertexBuffer& vertices,
    const Buffer& indices)
{
    return calculateBoundingBox<V, I>(
        drawElements, vertices.numComponents(), reinterpret_cast<const V*>(vertices.data()),
...
public:
    ...
    std::pair<uint32_t, uint32_t> addVertices(const std::unordered_map<VertexAttribute, const std::vector<glm::vec4>&>& vertices)
    {
        ...
        for (const auto& [attrib, buffer] : mesh->vertexBuffers())
        {
            ...
            {
                if (bufferIt->second.size() != addedVerticesCount) LOG_CRITICAL << "Buffer have different size";
...
void AbstractPainter::drawArrays(
    const std::unordered_map<VertexAttribute, const std::vector<glm::vec4>&>& vertices,
    PrimitiveType primitiveType)
{
    if (!m_->mesh) LOG_CRITICAL << "Mesh can't be nullptr";
...
void AbstractPainter::drawElements(
    const std::unordered_map<VertexAttribute, const std::vector<glm::vec4>&>& vertices,
    PrimitiveType primitiveType,
    ...
    DrawElementsIndexType drawElemetsIndexType)
{
    ...
    auto drawElemetsBuffer = std::make_shared<DrawElementsBuffer>(
        primitiveType, static_cast<uint32_t>(indices.size()), drawElemetsIndexType, firstCount.first);
...
{
    drawElements(
        {{VertexAttribute::Position, s_triangleVertices},
         {VertexAttribute::Normal, s_triangleNormals},
         {VertexAttribute::TexCoords, s_triangleTexCoords}},
        PrimitiveType::Triangles, s_triangleIndices, toDrawElementsIndexType<decltype(s_triangleIndices)::value_type>());
    if (m_->mesh->vertexBuffers().count(VertexAttribute::Tangent)) m_->calculateTangents();
    return *this;
}

MeshPainter& MeshPainter::drawPlane(const glm::vec2& size)
{
    const glm::vec4 v4Size(size, 0.f, 1.f);
    std::vector<glm::vec4> vertices(s_planeVertices.size());
    for (size_t i = 0u; i < s_planeVertices.size(); ++i)
        vertices[i] = s_planeVertices[i] * v4Size;

    drawElements(
        {{VertexAttribute::Position, vertices},
         {VertexAttribute::Normal, s_planeNormals},
         {VertexAttribute::TexCoords, s_planeTexCoords}},
        PrimitiveType::Triangles, s_planeIndices, toDrawElementsIndexType<decltype(s_planeIndices)::value_type>());
    if (m_->mesh->vertexBuffers().count(VertexAttribute::Tangent)) m_->calculateTangents();
    return *this;
}
...
{
    drawElements(
        {{VertexAttribute::Position, s_tetrahedronVertices},
         {VertexAttribute::Normal, s_tetrahedronNormals},
         {VertexAttribute::TexCoords, s_tetrahedronTexCoords}},
        PrimitiveType::Triangles, s_tetrahedronIndices, toDrawElementsIndexType<decltype(s_tetrahedronIndices)::value_type>());
    if (m_->mesh->vertexBuffers().count(VertexAttribute::Tangent)) m_->calculateTangents();
    return *this;
}
...
MeshPainter& MeshPainter::drawCube(const glm::vec3& size)
{
    ...
    drawElements(
        ...
        PrimitiveType::Triangles, s_cubeIndices, toDrawElementsIndexType<decltype(s_cubeIndices)::value_type>());
    if (m_->mesh->vertexBuffers().count(VertexAttribute::Tangent)) m_->calculateTangents();
    return *this;
}
...
{
    drawElements(
        {{VertexAttribute::Position, s_suzanneVertices},
         {VertexAttribute::Normal, s_suzanneNormals},
         {VertexAttribute::TexCoords, s_suzanneTexCoords}},
        PrimitiveType::Triangles, s_suzanneIndices, toDrawElementsIndexType<decltype(s_suzanneIndices)::value_type>());
    if (m_->mesh->vertexBuffers().count(VertexAttribute::Tangent)) m_->calculateTangents();
    return *this;
}
...
{
    drawElements(
        {{VertexAttribute::Position, s_bunnyVertices},
         {VertexAttribute::Normal, s_bunnyNormals},
         {VertexAttribute::TexCoords, s_bunnyTexCoords}},
        PrimitiveType::Triangles, s_bunnyIndices, toDrawElementsIndexType<decltype(s_bunnyIndices)::value_type>());
    if (m_->mesh->vertexBuffers().count(VertexAttribute::Tangent)) m_->calculateTangents();
    return *this;
}
...
{
    drawElements(
        {{VertexAttribute::Position, s_teapotVertices},
         {VertexAttribute::Normal, s_teapotNormals},
         {VertexAttribute::TexCoords, s_teapotTexCoords}},
        PrimitiveType::Triangles, s_teapotIndices, toDrawElementsIndexType<decltype(s_teapotIndices)::value_type>());
    if (m_->mesh->vertexBuffers().count(VertexAttribute::Tangent)) m_->calculateTangents();
    return *this;
}
...
MeshPainter& MeshPainter::drawSphere(uint32_t segs)
{
    ...
    drawElements(
        {{VertexAttribute::Position, vertices}, {VertexAttribute::Normal, normals}, {VertexAttribute::TexCoords, texCoords}},
        PrimitiveType::Triangles, indices, toDrawElementsIndexType<decltype(indices)::value_type>());
    if (m_->mesh->vertexBuffers().count(VertexAttribute::Tangent)) m_->calculateTangents();
    return *this;
}
...
{
    drawElements(
        {{VertexAttribute::Position, s_cameraVertices}}, PrimitiveType::Lines, s_cameraIndices,
...
{
    drawElements(
        {{VertexAttribute::Position, s_boundingBoxVertices}}, PrimitiveType::Lines, s_boundingBoxIndices,
...
{
    drawArrays({{VertexAttribute::Position, s_screenQuadVertices}}, PrimitiveType::TriangleStrip);
    return *this;
}
