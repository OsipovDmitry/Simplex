...
template <>
float convertToPixelComponentType<float, uint8_t>(uint8_t value)
{
    return static_cast<float>(value) / std::numeric_limits<uint8_t>::max();
}

template <>
float convertToPixelComponentType<float, uint16_t>(uint16_t value)
{
    return static_cast<float>(value) / std::numeric_limits<uint16_t>::max();
}

template <>
uint8_t convertToPixelComponentType<uint8_t, uint16_t>(uint16_t value)
{
    return static_cast<uint8_t>(value >> 8u);
}

template <>
uint8_t convertToPixelComponentType<uint8_t, float>(float value)
{
    return static_cast<uint8_t>(glm::clamp(value, 0.f, 1.f) * std::numeric_limits<uint8_t>::max());
}

template <>
uint16_t convertToPixelComponentType<uint16_t, float>(float value)
{
    return static_cast<uint16_t>(glm::clamp(value, 0.f, 1.f) * std::numeric_limits<uint16_t>::max());
}

template <>
uint16_t convertToPixelComponentType<uint16_t, uint8_t>(uint8_t value)
{
...
{
    glm::vec<4, T> operator()(const glm::vec<1, T>& value) const
    {
        return glm::vec<4, T>(value.r, zero<T>(), zero<T>(), one<T>());
    }
};
...
glm::vec<Dst, T>* convertToNumComponents(uint32_t w, uint32_t h, uint32_t n, const uint8_t* d)
{
    glm::vec<Dst, T>* result = nullptr;
...
template <typename T>
uint8_t* convertToNumComponents(uint32_t w, uint32_t h, uint32_t n, const uint8_t* d, uint32_t newNumComponents)
{
    uint8_t* result = nullptr;
...
uint8_t* convertToNumComponents(uint32_t w, uint32_t h, uint32_t& n, PixelComponentType pct, const uint8_t* d, uint32_t newN)
{
    uint8_t* result = nullptr;
...
template <typename T>
T* convertToSize(uint32_t w, uint32_t h, uint32_t n, const T* d, uint32_t newW, uint32_t newH)
{
    return nullptr;
}

template <>
float* convertToSize(uint32_t w, uint32_t h, uint32_t n, const float* d, uint32_t newW, uint32_t newH)
{
    ...
    if (!stbir_resize_float_generic(
            d, w, h, 0, result, newW, newH, 0, n, n == 4u ? 3u : STBIR_ALPHA_CHANNEL_NONE, 0, STBIR_EDGE_CLAMP,
            STBIR_FILTER_DEFAULT, STBIR_COLORSPACE_LINEAR, allocContext))
    {
        std::free(result);
        result = nullptr;
    }
...
template <>
uint8_t* convertToSize(uint32_t w, uint32_t h, uint32_t n, const uint8_t* d, uint32_t newW, uint32_t newH)
{
    void* allocContext = nullptr;
...
template <>
uint16_t* convertToSize(uint32_t w, uint32_t h, uint32_t n, const uint16_t* d, uint32_t newW, uint32_t newH)
{
    void* allocContext;
...
uint8_t* convertToSize(uint32_t& w, uint32_t& h, uint32_t n, PixelComponentType pct, const uint8_t* d, uint32_t newW, uint32_t newH)
{
    uint8_t* result = nullptr;
    ...
    if ((newW != w) || (newH != h))
    {
        switch (pct)
        {
            ...
            {
                result =
                    reinterpret_cast<uint8_t*>(convertToSize<uint8_t>(w, h, n, reinterpret_cast<const uint8_t*>(d), newW, newH));
                break;
            }
            case PixelComponentType::Uint16:
            {
                result = reinterpret_cast<uint8_t*>(
                    convertToSize<uint16_t>(w, h, n, reinterpret_cast<const uint16_t*>(d), newW, newH));
                break;
            }
            default:
                break;
