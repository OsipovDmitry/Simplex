...
#include "header/riff_header.h"
#include "header/fmt_header.h"
#include "header/data_header.h"
#include "header/wave_header.h"
...
Error File::Read(uint64_t frame_number, void (*decrypt)(char *, size_t), void *output)
{
    if (!impl_->istream.is_open()) {
      return kNotOpen;
    }
    ...
    uint16_t bytes_per_sample = impl_->header.fmt.bits_per_sample / 8;
    ...
    if (bytes_per_sample == 1u)
        for (size_t sample_idx = 0; sample_idx < requested_samples; ++sample_idx)
        {
            ...
            impl_->istream.read(reinterpret_cast<char*>(&value), sizeof(value));
            decrypt(reinterpret_cast<char*>(&value), sizeof(value) / sizeof(char));
            memcpy(p + sample_idx * bytes_per_sample, &value, sizeof(value));
        }
