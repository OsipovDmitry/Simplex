...
inline int32_t readIntImpl(const rapidjson::Document::ValueType* value, int32_t defaultValue)
{
    ...
    else if (value->IsString())
    {
        ...
        if (valueString == "max") result = utils::maximum<T>();
...
{
    static const auto oneStep = [](const rapidjson::Document::ValueType* curDoc, const std::string& path, size_t& current,
                                   size_t& previous) -> const rapidjson::Document::ValueType*
    {
        const auto member = path.substr(previous, current - previous);

        if (!curDoc->HasMember(member.c_str())) return nullptr;
    ...
    if (curDocument) curDocument = oneStep(curDocument, path, current, previous);
    ...
    , m_path(path)
{
    ...
    if (!m_document.IsObject()) LOG_CRITICAL << "Settings file \"" << absoluteFilename << "\" is damaged";
